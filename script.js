...
contactForm.addEventListener('submit', function(e) {
    ...
    const subject = encodeURIComponent(`Order - ${data.orderType || 'General'} - CHOP'D`);
    const body = encodeURIComponent(`Name: ${data.name}\nPhone: ${data.phone}\nEmail: ${data.email || 'N/A'}\nOrder Type: ${data.orderType || 'N/A'}\nParty Size: ${data.partySize || 'N/A'}\nDetails: ${data.message || 'N/A'}\n\nPlease confirm my order.`);
    window.location.href = `mailto:info@chopdsalads.com?subject=${subject}&body=${body}`;
    const btn = this.querySelector('button[type="submit"]'); const orig = btn.innerHTML;
    btn.innerHTML = '&#10003; Order Submitted!'; btn.style.background = '#10b981'; btn.style.borderColor = '#10b981';
...
document.querySelectorAll('a[href^="tel:"]').forEach(link => { link.addEventListener('click', () => { if (typeof gtag === 'function') gtag('event', 'click_to_call', { business: "CHOP'D" }); }); });
